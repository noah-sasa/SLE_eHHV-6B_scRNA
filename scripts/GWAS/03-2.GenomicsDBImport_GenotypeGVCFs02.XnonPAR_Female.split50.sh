...
### GenomicsDBImport
mkdir -p tmp
mkdir -p tmp/tmp_genomicsdbimport_XnonPAR_Females_${SGE_TASK_ID}
mkdir -p gvcfs_db
/usr/bin/time -f "Memory:%M KB time:%E" -o log/GenomicsDBImport_XnonPAR_Females_${SGE_TASK_ID}.txt \
${GATK_DIR}/gatk --java-options "-Xmx24g -Xms24g -XX:+UseSerialGC"\
 GenomicsDBImport\
 --reference ${REF} ${gvcf_files_females}\
 --intervals ${interval_list}\
 --genomicsdb-workspace-path gvcfs_db/gvcfs_db_contig_XnonPAR_Females_${SGE_TASK_ID}\
 --overwrite-existing-genomicsdb-workspace true\
 --tmp-dir tmp/tmp_genomicsdbimport_XnonPAR_Females_${SGE_TASK_ID}\
 > log/GenomicsDBImport_XnonPAR_Females_${SGE_TASK_ID}.log 2>&1

rm -rf tmp/tmp_genomicsdbimport_XnonPAR_Females_${SGE_TASK_ID}

### GenotypeGVCFs
mkdir -p tmp/tmp_genotypegvcfs_XnonPAR_Females_${SGE_TASK_ID}
/usr/bin/time -f "Memory:%M KB time:%E" -o log/GenotypeGVCFs_XnonPAR_Females_${SGE_TASK_ID}.txt \
${GATK_DIR}/gatk --java-options "-Xmx24g -Xms24g -XX:+UseSerialGC"\
 GenotypeGVCFs\
 --reference ${REF}\
 --variant gendb://gvcfs_db/gvcfs_db_contig_XnonPAR_Females_${SGE_TASK_ID}\
 -L ${interval_list}\
 --output JointGenotyping.XnonPAR_Females_${SGE_TASK_ID}.vcf.gz\
 --tmp-dir tmp/tmp_genotypegvcfs_XnonPAR_Females_${SGE_TASK_ID}\
 > log/GenotypeGVCFs_XnonPAR_Females_${SGE_TASK_ID}.log 2>&1

rm -rf tmp/tmp_genotypegvcfs_XnonPAR_Females_${SGE_TASK_ID}
